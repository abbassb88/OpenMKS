...
bool AN_PCL_SCP2(char* objectfile, char* scenefile)
{
	bool B = false;
	// Point clouds
	...
	if (pcl::io::loadPCDFile<PointNT>(objectfile, *object) < 0 ||
		pcl::io::loadPCDFile<PointNT>(scenefile, *scene) < 0)
	{
	...
	if (align.hasConverged())
	{
		...
		m_pc[0].clear();
		for (int i = 0; i < object_aligned->width; i++)
		{
			P = object_aligned->at(i);
			m_pc[0].addPointToFiltered(P.x, P.y, P.z, 1, 1, 1);
		}
		...
		{
			P = scene->at(i);
			m_pc[1].addPointToFiltered(P.x, P.y, P.z, 1, 1, 1);
		}
	}
	else
	{
